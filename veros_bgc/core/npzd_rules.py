...
def primary_production(vs, nutrient, plankton):
    ...
    return {
        nutrient: -vs.redfield_ratio_PN * vs.net_primary_production[plankton],
...
def excretion(vs, zooplankton, nutrient):
    ...
    return {
        zooplankton: -vs.excretion_total,
        nutrient: vs.redfield_ratio_PN * vs.excretion_total,
    }


def primary_production_from_DIC(vs, nutrient, plankton):
    """Stub for primary production consuming DIC.

    This should be replaced by a common sink rule.
    """
...
def calcite_production(vs, plankton, DIC, calcite):
    ...
    dprca = (
        (
            vs.mortality[plankton]
            + vs.grazing[plankton] * (1 - vs.assimilation_efficiency)
        )
        * vs.capr
...
def calcite_production_phyto(vs, DIC, calcite):
    ...
    How much depends on the mortality and sloppy feeding of and on phytoplankton
    and zooplankton.
    """
...
def dic_alk_scale(vs, DIC, alkalinity):
    ...
    return {
        alkalinity: (vs.temporary_tracers[DIC] - vs.npzd_tracers[DIC][:, :, :, vs.tau])
        / vs.redfield_ratio_CN
    }
